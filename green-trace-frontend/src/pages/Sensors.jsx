...
const STATUS_BADGE = {
  ...
  'Low Battery': 'text-amber-400 bg-amber-500/10',
  Offline: 'text-slate-500 bg-slate-500/10'
}

const FILTERS = ['All', 'Active', 'Alert', 'Low Battery', 'Offline']
...
export default function Sensors() {
  ...
  const { data, loading, error } = useApiData('/sensors', 1000)
  if (loading || !data) return <div><TopBar title="Canopy Dashboard" subtitle="AI Forest Monitoring · Real-time" /><div className="p-6 md:p-10 text-sm text-slate-500">Loading live sensor data...</div></div>
  ...
  const { sensorUnitStats, sensorRegistry } = data
