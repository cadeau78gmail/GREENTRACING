...
import { forestZoneStats, forestZones } from '../data/mockData.js'
          ...
          {forestZones.map((zone, index) => (
            <div key={zone.name} className="card overflow-hidden transition-all duration-200 ease-out hover:-translate-y-1 hover:border-[#b8d0c1] hover:bg-[#f4faf5] hover:shadow-[0_10px_22px_rgba(15,23,42,0.06)]">
              <div className="relative h-28 p-4 flex items-start justify-between" style={{ backgroundColor: theme.colors.accent }}>
                <p className="font-serif text-lg text-white drop-shadow">Zone {String(index + 1).padStart(3, '0')}</p>
              ...
              <div className="p-5">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <p className="font-serif text-xl text-slate-50">Zone {String(index + 1).padStart(3, '0')}</p>
                    <p className="text-sm text-slate-500 flex items-center gap-1 mt-0.5">
                      <MapPin size={12} /> Node {String(index + 1).padStart(3, '0')}
                    </p>
                  </div>
